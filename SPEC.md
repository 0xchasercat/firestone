...
4. **Cross-origin.** A non-GET request must carry `Sec-Fetch-Site: same-origin`, or a matching `Origin` when `Sec-Fetch-Site` is absent. Non-browser clients that send neither are already token-gated.

Failures answer `401` (token) or `403` (host, origin) with the same `ErrorEnvelope` shape as every other error. No `WWW-Authenticate` header: it would raise a browser credential dialog for a token this UI already holds.
