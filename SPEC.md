...
- Sidecar reads are capped at 64 KiB and machine-state reads at 1 MiB from the already-open no-follow descriptor. One `images/.lock` serializes pull publication, machine image pinning, remove, and prune. Two requests for the same reference are deduplicated by that lock rather than refused: a pull, and a machine start that may pull, wait long enough to outlast the download, conversion and packing deadlines another pull may be holding the lock through, then take the cache-hit path the first one published. Only the operations that cannot pull keep the ordinary ten-second wait and its `busy` refusal. New owned directories and locks are forced to 0700/0600 despite umask. A current-user regular lock whose owner permissions are a strict subset of 0600 is recovered to 0600 after an interrupted creation; wrong-owner, special, symlinked, owner-executable, or group/world-accessible existing locks are rejected unchanged.
...
- **Confirmation.** With an interactive terminal and without `--yes`, the prompt lists the exact paths first. `--purge` is confirmed a second time, because it destroys machines and images that no reinstall brings back.
...
| `uninstall` removes the binary and, only when asked, the data (P2-CORE) | CLI-only, no action and no REST route; the target is the running executable, refused under `/usr` or `/opt` or in a directory this user cannot write; the default keeps machines and images and says where they are; `--purge` deletes the config, data, and runtime directories, is refused while a machine runs, and is confirmed twice | expose it as an action so REST could uninstall too; purge by default because an uninstall "should" leave nothing; delete a `/usr` copy anyway when the user happens to have write access | A running server accepting a request to delete its own binary is a hole with no legitimate caller, so this one never becomes an action. Data outlives an uninstall by default because a reinstall is common and a deleted VM is not recoverable; the reader is told exactly where it lives and how to remove it. A path under a package manager's prefix belongs to that package manager, and removing the file behind its back leaves a broken package rather than an uninstalled program. |
