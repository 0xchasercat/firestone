...
| M5-03 gates and unavailable KVM evidence | Require local stable and Rust 1.85.0 format, Clippy with warnings denied, and the complete suite; live x86_64 release-metadata/checksum retrieval; Python validator tests; generated-help command and local-link checks; real unprivileged Ubuntu, Fedora, and Arch container rows; final-head GitHub Ubuntu Rust CI plus the distro workflow; and only a gated skip when KVM is unavailable. | targeted Rust tests; one toolchain; source HTTP checks as boot evidence; fake `/dev/kvm` as KVM; omit a failed duplicate run | Both local toolchains passed format, Clippy, and all 586 tests. Fifteen Python tests passed; five x86_64 catalog metadata/checksum pairs passed; 52 documented Firestone commands and six local links passed; and all three local distro rows passed with no KVM. At implementation commit `a9aad5f`, GitHub Ubuntu Rust run [33276918476](https://github.com/0xchasercat/firestone/actions/runs/33276918476) and the four-job catalog/docs/doctor run [33276918451](https://github.com/0xchasercat/firestone/actions/runs/33276918451) passed. A simultaneous push-triggered Rust run [33276901020](https://github.com/0xchasercat/firestone/actions/runs/33276901020) hit `starting_recovery_timeout_terminates_the_pinned_vmm`; the PR-triggered run of the same commit passed that full suite. E2E 11 ran only in disabled mode because the recorded KVM host is unreachable, so verify 3 and every new boot claim remain open. |
