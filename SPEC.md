...
- Sidecar reads are capped at 64 KiB and machine-state reads at 1 MiB from the already-open no-follow descriptor. One `images/.lock` serializes pull publication, machine image pinning, remove, and prune. Two requests for the same reference are deduplicated by that lock rather than refused: a pull, and a machine start that may pull, wait long enough to outlast the download, conversion and packing deadlines another pull may be holding the lock through, then take the cache-hit path the first one published. A request that does not get the lock at once logs one line saying it is waiting for the image operation already running. Only the operations that cannot pull keep the ordinary ten-second wait and its `busy` refusal. New owned directories and locks are forced to 0700/0600 despite umask. A current-user regular lock whose owner permissions are a strict subset of 0600 is recovered to 0600 after an interrupted creation; wrong-owner, special, symlinked, owner-executable, or group/world-accessible existing locks are rejected unchanged.
...
**Coverage.** `shell`, `ssh-config` and `cp` refuse through the shared sidecar-kind check, and `start` skips the readiness wait for a direct-kernel machine. The one SSH-shaped surface outside this rule is the §16.3 shell WebSocket transport, which allocates its session after the upgrade and ends it with its own transport close rather than a pre-flight refusal.
...
| `doctor [--fix]` | diagnose the host and apply every repair that needs no administrator; `--fix` additionally offers the AppArmor repair that asks for elevation (§17.3) |
| `system metrics` | one sample of host CPU, memory, and data-directory space (§25.5) |
| `uninstall [--purge]` | remove the firestone executable; `--purge` also deletes the config, data, and runtime directories (§15.7) |
| `serve [--listen unix:PATH|tcp:HOST:PORT] [--host ADDR] [--port PORT] [--token FILE]` | REST listener; a TCP listener must be loopback and must carry a token. `--host`/`--port` are sugar for one `--listen tcp:` form and cannot be combined with it |
...
### 15.7 `uninstall` (normative)

`firestone uninstall` removes the installation the running executable belongs to. It is CLI-only (§5.2): there is no action and no REST route, because a REST client asking a server to delete its own binary is a hole, not a feature.

- **Target.** The path comes from the running process, not from `PATH`. A path under `/usr` or `/opt` is refused with `usage`, naming the prefix and saying to use the package manager that installed it. A path whose directory this user cannot write is refused the same way. Neither refusal deletes anything.
- **Confirmation.** Without `--yes`, the prompt lists the exact paths first, and `--purge` is confirmed a second time because it destroys machines and images that no reinstall brings back. A run that cannot ask — no terminal on stdin and stderr, or `--json` — is refused with `usage` and removes nothing. `--yes` is the only way to uninstall without answering the question.
- **Scope.** The default run removes the executable and keeps everything else, and says where the machines and images it kept still live. `--purge` also removes the config, data, and runtime directories, in that order, before the executable, so a failed removal leaves a working `firestone` to retry with.
- **Refusals.** `--purge` is refused with `conflict` while any machine is running, listing them, because deleting the data directory under a running VMM leaves a process with no state.
- **Result.** One `Result` with action `uninstall` and payload `{removed: [path], kept: [{path, holds}]}`, rendered as a `Removed:`/`Kept:` list or emitted unchanged under `--json`.
...
- `--host ADDR` and `--port PORT` are sugar for the same listener: `--port 8080` alone means `--listen tcp:127.0.0.1:8080`, and `--host` chooses which loopback address to bind. Either flag with `--listen` is a usage error naming both. `--host` needs `--port`, because an address without a port names no listener. The sugar changes nothing else: a non-loopback `--host` is refused with the same message `--listen tcp:` gives, and the listener still requires `--token FILE`.
...
[`docs/openapi.json`](docs/openapi.json) is the dependency-free OpenAPI 3.1 contract for the existing routes. It records exact request and response schemas, both transports, aggregation and stream framing, limits, statuses, nullability, and examples. It is a static artifact, not a runtime endpoint. A behavior-level test parses it and compares its 29 explicitly authored operations with the configured axum router; axum's synthesized `HEAD` handling for `GET` remains framework behavior rather than a separately authored Firestone operation.
...
| GET | `/v1/host/metrics` | | `HostMetricsResult` (§25.5); needs no machine |
...
Every response carries `default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:; font-src 'self'; connect-src 'self'; frame-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'`, plus `Referrer-Policy: no-referrer`, `X-Content-Type-Options: nosniff` and same-origin `Cross-Origin-Opener-Policy`/`Cross-Origin-Resource-Policy`. There is no `'unsafe-inline'` and no `'unsafe-eval'`. Consequently the UI uses no inline `style` attribute, no inline script, and none of htmx's `new Function()`-compiled constructs (trigger filters, `hx-on:*`, `js:` values). Everything they would have done is done in `app.js` with delegated listeners, and every show/hide is the `hidden` attribute.

`frame-src 'self'` is on every response rather than on the one screen that frames anything, because htmx navigation keeps the document that first loaded: a policy attached to `/machines/{name}` would not apply once the reader reached that screen from `/machines`. It permits same-origin frames and grants no script capability.

**The terminal exceptions.** Two responses in the application, and no others, carry a wasm-capable policy.

- The standalone terminal page carries the base policy with `script-src 'self' 'wasm-unsafe-eval'`. It differs by exactly one token. `'wasm-unsafe-eval'` is what permits `WebAssembly.compile`; it is *not* `'unsafe-eval'`, so `eval` and `new Function` stay forbidden on that page too, and script sources stay same-origin-only. A page carrying this policy can only instantiate WebAssembly bytes it already fetched from Firestone's own asset table, so exploiting the exception still requires prior injection into the executable — the relaxation widens what a compromised page could do, never how a page becomes compromised.
- The framed variant, `…/terminal?embed=1`, carries that policy with `frame-ancestors 'self'` rather than `'none'`, and differs from it by exactly that one token. It is the only response in the application that may be framed at all, and only by this same origin. The standalone page stays unframeable.

The flag is one exact value, `embed=1`. Every other spelling is the standalone page, because this query decides which policy a response carries and a lenient parse there would be a way to reach the framable variant by accident.

The mechanism is a response marker, not a path prefix: the terminal handler attaches a private marker to the response it built, and the shared security-headers middleware selects the policy the marker names. A route cannot acquire a weaker policy by being renamed, moved, matched by a wildcard, or by failing — the terminal handler's own error path renders through the normal shell and carries the strict policy, for both variants. Regression tests assert all three policies byte for byte.
...
An absent *value* is not an empty state. A figure with no sample behind it renders as an em dash, and a forward set with no members renders as a single em-dash chip. The detail spec tab is the exception and renders words rather than dashes or tokens: see §16.5.9.

**Copy.** A page subtitle says what the page holds, in a fragment. An empty state names what is absent and says in one line what would put something there. Neither explains how Firestone works: the reader has already opened the screen, and a paragraph about reconciliation, image pinning or snapshot tiers at the top of it is a lesson nobody asked for. What a destructive action costs is stated in the dialog that asks for it, where the reader is deciding, and nowhere else.
...
**Before the first start (normative).** A machine that has never run has written nothing, which is an ordinary state and not a failure. The logs tab renders an empty state saying so, and nothing on its load path produces a status a browser would notify about: a read that comes back empty and a read the dispatcher refuses render the same quiet empty state. The `<pre>` is still there, hidden, so the follow stream has somewhere to write the first byte into; the first byte retires the empty state.
...
`GET /machines/{name}/terminal` is a full-window browser terminal for one machine, with one slim bar carrying the machine name, its status, Console and Shell tab buttons, a connection pill, and a link back to the machine. It is a read like every other screen: rendering it dispatches `Action::Show`. The page renders for a machine in any state, because a terminal that cannot attach must say why rather than 404. The palette offers a Terminal entry only while a machine is running.

**The Console tab (normative).** A running machine's detail page carries a `console` tab, and the detail head's Terminal control opens it rather than leaving the page: attaching to a machine is one click from the machine you are already looking at. The tab is offered only while the machine is running, and `?tab=console` on a machine in any other state resolves to the spec tab, so the strip and the panel cannot disagree.

The panel is an `<iframe>` of `…/terminal?embed=1`, not a second terminal rendered inline. That is what keeps `'wasm-unsafe-eval'` on the terminal document: rendering the emulator on the detail page would put the wasm token on the screen that holds every other read. The embed variant is the same page with its chrome replaced by one expand control; the detail page loads no terminal client and no emulator of its own.

Expanding navigates the top window to the standalone page. `term.js` tears the session down first — closing the socket while the framed document is still alive, which is the earliest a browser will send a close frame — and then navigates. The handoff is still two connections either side of one server-side close, so a slow close can leave the standalone page briefly on the `BUSY` overlay that §16.3 already defines. Its Reconnect button is the retry, and nothing reconnects on its own, because retaking a single-client console is an operator's decision.
...
**Host usage (normative).** The overview carries one compact strip beside its allocation cards: host CPU per cent with a sparkline, host memory used against total with a meter, and free space on the data disk with a meter. It reads one route, `GET /v1/host/metrics`, every 5 s, and derives its history in the browser exactly as the machine strip does — a 60-sample ring buffer per tab, nothing stored on the host. A percentage the payload reports is used as reported; otherwise it is derived from two samples of the cumulative counters, and a counter that went backwards is dropped rather than drawn.

The strip is served **hidden** and reveals itself only once a sample has parsed. A build whose server does not answer that route, or a read that starts failing, leaves it hidden and raises nothing: a strip of em dashes claims a measurement that was never taken, and a host without the route is not an incident. The allocation cards are unchanged and keep their allocation semantics.
...
**Spec tab rendering (normative).** The spec tab is read by a person, so it renders values rather than the spec's JSON tokens. An absent optional is `not set`, a list with no members is `none`, a flag is `on` or `off`, a path is shown as it is, and `cloud_init.password` is `set` or `not set`. Three rows answer with the fact behind the absence instead: an unset `arch` is the host's own architecture and the `(host default)` note, taken from the same version payload the sidebar reads; an unset `network.mac` is `generated at start`; an unset `vmm.binary` is `the embedded build`. A version read that failed leaves the architecture unknown, and the row then says only `host default` rather than inventing one.

The vmconfig tab follows the same rule. A machine that has never started has no published config, the dispatcher refuses that read by naming a file inside the machine directory, and that message is dropped: the tab renders its empty state and nothing else. A host path in an empty state reads as a fault where there is none.
...
- The detail spec tab reports `cloud_init.user_data_inline` as a byte count, `cloud_init.ssh_authorized_keys` as a count, and `cloud_init.password` as `set` or `not set` — never a length, never a value. Inline user-data content and password bytes never reach a rendered page.
...
The machine detail tab strip carries `spec`, `console` (while running, §16.5.6), `logs`, `snapshots` and `vmconfig`. One function resolves a requested tab name to the tab that will render, so the strip and the panel cannot disagree and exactly one tab is marked active for any name a URL can carry; an unknown name is the spec tab. The snapshots panel is a read of `Action::SnapshotList` — the same action `GET /v1/machines/{name}/snapshots` dispatches — rendered newest identifier first, with the snapshot, its tier, its creation instant, its disk size and its captured memory. `kind` renders as §23's word, `cold` or `warm`, never as a colour and never renamed, and a cold snapshot's memory renders as an em dash because it captured no memory rather than zero bytes.
...
Pins live in `deps.toml` in the repository (name, version, runtime architectures, immutable URL, sha256, source and license provenance). Checksums come from real downloaded or twice-built bytes. The x86_64 release build downloads and verifies Cloud Hypervisor, passt, and qemu-img before Cargo, then `build.rs` independently matches all three to `deps.toml` and embeds them with `include_bytes!`; a strict release build fails when any payload or hash is absent. First use publishes a versioned `<data>/bin/<name>-<version>` executable under a current-user lock with no-follow validation, exact length/hash/mode readback, fsync, and no-replace publication. An installed file that fails that validation against the payload this build carries is **superseded, not fatal**: the owned bin directory holds artifacts derived from the manifest, so the manifest that ships with the running binary wins, and the helper is rewritten in place through the same partial-write, hash-verified path and persisted over the old file. This is what lets an upgrade across a helpers rebuild work at all: an older release installed different bytes under the same install name, and refusing them would strand the user with an error naming two lengths. Every other guard still refuses: the destination must be a regular current-user-owned file in the owned bin directory, opened without following a symlink, at the expected mode, and the replacement is still hash-verified before and after publication. A file that already matches short-circuits, and an absent one is still published without clobbering, so two callers racing on identical bytes publish exactly once. A verified literal root-owned passt copy installed for AppArmor takes precedence. Development and compile-only aarch64 builds retain their existing installed/PATH/download fallbacks; the x86_64 standalone release requires no host VMM, passt, or qemu-img package.
...
Each check prints `ok`, `warn`, `fail`, or `fixed`, a one-line reason, and exact remediation.

**`doctor` repairs what needs no administrator.** A plain `firestone doctor` performs every ordinary current-user repair — creating the owned directories, downloading the vendored binaries, generating the SSH key — and then reports the host it leaves behind. A check the run repaired reports `fixed` and carries `fixed: true` in the payload; a check that was already well reports `ok` and omits the field. Nothing that would prompt for elevation happens here.

That is a CLI rule, not an action rule. `Action::Doctor { fix: false }` stays a read that changes nothing, and `GET /v1/doctor` still projects it, so a REST client and the web UI can poll the report without starting a download. The repair belongs to the `doctor` command in the CLI, which dispatches `Action::Doctor { fix: true }`. `--json` is an output format, not a different command: it repairs too, and a caller that wants a pure read uses the REST route.

`--fix` remains the spelling that additionally authorizes elevation. When Ubuntu AppArmor blocks passt's mandatory user namespace, an interactive `doctor --fix` first displays the literal root-owned helper/profile changes and requires an explicit confirmation; `--yes` never authorizes elevation. Confirmed repair invokes only `install` and `apparmor_parser` through `sudo` or `pkexec`, never changes a sysctl, and verifies helper hash/version/root ownership/non-writability, exact profile bytes, and loaded profile state. A plain `doctor`, a non-interactive run, or a declined prompt prints the exact administrator commands without invoking them and points at `doctor --fix`.
...
5. embedded `cloud-hypervisor v53.0` is present in x86_64 standalone builds and matches the pinned manifest hash; RHF and edk2 firmware remain vendored files → fix: the run materializes the VMM and downloads missing firmware
6. vendored `virtiofsd` present with the matching checksum → fix: downloaded by the run itself
...
11. firestone SSH key present → fix: generated by the run itself
...
| 26 | resolved | §11.8's SSH refusal and readiness rule hold | `refuse_ssh_surface` answers `shell`, `ssh-config` and `cp` from the machine's own sidecar kind, and the start path skips `wait_for_ssh_ready` for a direct-kernel machine (`wait && provisioning && !direct_kernel`). Unit coverage in `store.rs`; the M6 OCI E2E started container machines with the default wait and stopped them within the §9.4 force path on bare metal |
...
|---|---|---|---|
| `doctor` repairs instead of prescribing (P2-CORE) | A plain `firestone doctor` runs every repair that needs no administrator — owned directories, vendored downloads, the SSH key — and marks each repaired check `fixed`; the AppArmor repair, the only one that asks for elevation, stays behind `--fix`, and a plain run prints its exact commands. The change lives in the CLI command layer: `Action::Doctor { fix: false }` is still a read, so `GET /v1/doctor` and the UI's cached poll download nothing | keep prescribing `--fix` and leave the repair to a second run; make `Action::Doctor { fix: false }` repair, so REST repairs too; exempt `--json` from the repair so scripts keep a pure read | Telling a reader to run the same command again with one more flag is not a diagnosis, and every fix behind that flag is something Firestone would do on its own at first use anyway. Moving the repair into the action would put a multi-hundred-megabyte download behind a 15-second-cached UI poll, which is why the seam is the command and not the action. `--json` is an output format, not a different command; a caller that wants a guaranteed read has one, and it is the REST route. |
| `serve --host`/`--port` as sugar, not a second path (P2-CORE) | Both flags resolve to one `SocketAddr` before `resolve_serve_transport` runs, so the TCP branch, the loopback refusal, and the `--token` requirement are the same code they always were; `--host` requires `--port`, and either flag with `--listen` is a clap conflict naming both | let `--host` default the port; accept a hostname and resolve it; add a second listener branch for the sugar | The loopback rule is the whole security argument of §16.1, so the sugar must not be able to reach a different code path than `--listen tcp:`. An address with a defaulted port would make `firestone serve --host ::1` bind something the reader did not name. Resolving a hostname would let DNS choose the bind address, which is exactly the property the loopback rule exists to deny. |
| `uninstall` removes the binary and, only when asked, the data (P2-CORE) | CLI-only, no action and no REST route; the target is the running executable, refused under `/usr` or `/opt` or in a directory this user cannot write; the default keeps machines and images and says where they are; `--purge` deletes the config, data, and runtime directories, is refused while a machine runs, and is confirmed twice; a run that cannot print the question and read the answer — no terminal, or `--json` — is refused with `usage` unless `--yes` is given | expose it as an action so REST could uninstall too; purge by default because an uninstall "should" leave nothing; delete a `/usr` copy anyway when the user happens to have write access; skip the prompt when there is no terminal, the way `prune` and `remove` do | A running server accepting a request to delete its own binary is a hole with no legitimate caller, so this one never becomes an action. Data outlives an uninstall by default because a reinstall is common and a deleted VM is not recoverable; the reader is told exactly where it lives and how to remove it. A path under a package manager's prefix belongs to that package manager, and removing the file behind its back leaves a broken package rather than an uninstalled program. Skipping the prompt is safe for `prune` and `remove` because the dispatcher still refuses without `--force`; `uninstall` has no such backstop, so `firestone uninstall --purge </dev/null` would delete every machine and image with nothing asked, and a reader piping `n` into it would be deleted by their own refusal. |
| An installed helper that fails validation is superseded, not fatal (P2-CORE) | Under the existing helper lock, a file in the owned bin directory whose length or hash does not match this build's embedded payload is treated as an artifact an older release installed under the same install name, and is replaced in place through the same hash-verified partial write with a plain persist; every other guard — owned directory, no-follow, mode, readback hash — still refuses | keep the hard error and tell the user to delete the file; version the install name by payload hash so old and new never collide; skip validation when a file exists | The bin directory holds artifacts derived from the manifest the running binary carries, so the manifest is the authority over its contents; the alternative stranded every user who upgraded across a helpers rebuild behind an error naming two lengths. Hashing the install name would leave the old bytes on disk forever and make the version in the path a lie. The no-replace publication stays for the absent case, which is the race it was written for. |
| Same-reference pulls wait instead of failing `busy` (P2-CORE) | `pull`, and the machine-start path that may pull, take the image store lock with a deadline that outlasts the download, conversion and packing deadlines one pull can hold it through; the second request then reaches the cache-hit path the first published, and logs one line saying it is waiting so a long wait is never silent. Every operation that cannot pull keeps the ten-second wait | keep the ten-second wait everywhere; return a distinguishable "already in progress" payload for the second caller; deduplicate in the web UI by disabling the button | The 409 the UI showed during a successful pull was not a conflict: it was the ordinary lock wait expiring after ten seconds while a legitimate multi-minute download held the lock. A distinguishable payload would still make the caller handle a failure for work that is going to succeed. Fixing it in the UI would leave every other client — a second terminal, a create racing a pull — with the same false conflict. |
| Framing the terminal rather than relaxing the detail page (P2-UI) | The Console tab on a running machine's detail page is an `<iframe>` of `…/terminal?embed=1`. That one response carries the wasm policy with `frame-ancestors 'self'`; `frame-src 'self'` is added to every policy the application serves, and nothing else changes. Expanding tears the session down and then navigates the top window | render the emulator inline on the detail page and give that screen `'wasm-unsafe-eval'`; put `frame-src 'self'` on the detail screen's response only; keep the Terminal control pointing at the full page and add no tab; open the full page in a new window instead of expanding | Attaching to a machine was a navigation away from everything else about it, which is the wrong default for the thing operators reach for most. Rendering the emulator inline would put the wasm token on the screen that holds every other read, which is exactly the blast radius §16.5.2 spent a wave keeping to one document; framing confines it by construction. `frame-src` cannot be scoped to one screen because htmx navigation keeps the first document that loaded, so a reader arriving at `/machines/web` from `/machines` would carry the policy of `/machines` — and `frame-src 'self'` grants no script capability, which is why widening it uniformly is the cheaper half of the trade. Expanding tears down first because the console broker is single-client and the CLI shares it: closing while the framed document is alive is the earliest a browser sends a close frame, and what remains is a brief `BUSY` the existing overlay already names and offers a retry for |
| The spec tab answers in words, not in JSON tokens (P2-UI) | Absent optionals render `not set`, empty lists `none`, flags `on`/`off`; `arch` renders the host's own architecture with a `(host default)` note, `network.mac` renders `generated at start`, `vmm.binary` renders `the embedded build`. The host architecture is read server-side from the version payload the sidebar already reads | keep the spec's own tokens, since the tab is labelled "spec"; render an em dash for every absent value; resolve the host architecture in the browser from `/v1/version`; add a "raw" toggle showing the JSON | A tab of `null`, `[]`, `true` and `unset` is a debug dump wearing a label, and the one row people actually asked about — `arch: null (host)` — was withholding an answer the server already had. An em dash is the right token for a measurement that was not taken (§16.5.4) and the wrong one for a value that is simply not configured, which is a fact the machine has. Resolving the architecture in the browser would put a second read and a second projection behind a row the server renders anyway, and a raw toggle would ship two renderings of one spec to avoid choosing one |
| The host usage strip hides rather than degrades (P2-UI) | The overview's host strip is served hidden and reveals itself only after `GET /v1/host/metrics` parses; a failing or absent route hides it again, backs the poll off to 15 s, and raises nothing | render the strip with em dashes until the route answers; toast when the read fails; drop the strip when the route 404s and never poll again; render it server-side from a shared result | A strip of em dashes claims three measurements were taken and came back empty, when in fact none were attempted; hiding claims nothing. The route belongs to another agent's change, so a UI that toasted its absence would report an incident on every host running a build that predates it. Backing off rather than giving up means a `serve` that gains the route recovers without a reload, which is the same rule §16.5.7 already applies to a machine that stopped. Server-side rendering would make the strip a polled fragment, and a swap would throw away the ring buffer the sparkline is drawn from |
| Page copy states what a screen holds and stops (P2-UI) | Every page subtitle is a fragment naming the contents; every empty state is a title plus one line saying what would put something there. What a snapshot costs and what a restore replaces moved into those two dialogs. The overview prints one version string with the release and commit on its hover title | keep the explanatory subtitles, since they were accurate; move the explanations into a help page; keep the long empty states and shorten only the subtitles; keep the full version identity in the footer line | Every one of these strings was true and none of them was wanted: a reader who opened the machines list did not ask how reconciliation works, and a snapshot empty state that explains restores is teaching a feature the reader has not reached. The facts worth keeping are the ones that change a decision, and those belong beside the button that makes it, which is where the snapshot and restore dialogs already were. The version line said `firestone 0.2.1 · release v0.2.1 · <sha>` — three spellings of one number — so the line keeps the number and the title keeps the provenance |
...
| Documenting the OCI SSH gap instead of closing it (M6-31) | `docs/images.md` states that `firestone start` on an OCI machine waits for an sshd that does not exist and that `--no-wait` is the spelling that works, and that `shell` and `cp` cannot connect; §11.8 gains a "not implemented in v0.2" paragraph and §20 gains verify 26 | implement the readiness and refusal branches inside the documentation task; leave §11.8 reading as though it shipped; omit the behavior from the guide and let the timeout teach it | A documentation pull request that changes start-path behavior is a behavior change wearing a docs label, and it would land unreviewed against two concurrent feature branches. The alternative that costs nothing is worse: a guide written from §11.8 would tell every reader to run a command that hangs until its deadline. Recording the gap in three places — the guide for the user, §11.8 for the implementer, verify 26 for the acceptance run — keeps the contract intact and the cost visible. |
| Docs validator resolves `snapshot` and `system` subcommands (M6-31) | `scripts/validate-docs.py` gains `SUBCOMMAND_GROUPS = {"images", "snapshot", "system"}` and resolves a group's subcommand before asking the binary for help, so `firestone system prune --dry-run` is checked against `firestone system prune --help` | keep the hardcoded `images` case and write every grouped command without its flags; special-case each new group with its own branch; drop the flag check for grouped commands | The validator's whole value is that a flag in the guide has to exist in the binary. With only `images` resolved, `--dry-run`, `--machines`, `--force` and `--start` could not appear in any documented command, so the guide would either omit the flags that make prune and snapshot restore usable or silently stop checking them. One set replaces one hardcoded name and stays correct for a fourth group. |
...
Snapshots in two tiers (§23: cold guaranteed, warm verified through `vm.pause`/`vm.snapshot`/`vm.restore` and the shim's restore launch mode), `clone` (§24), disk grow and live cpu/memory resize inside opt-in `cpus_max`/`memory_max` headroom (§9.5), per-machine metrics sampling (§25), `cp` (§11.9), system prune (§26), pending-forward surfacing (§12.5), cloud-init password, inline-key and inline-user-data authoring (§10.5), OCI image boot (§8.5, §8.6, §9.5, §10.5: registry pull, layer merge, ext4 rootfs packing, pinned direct-boot kernel, and `firestone-init` as PID 1, embedded in a standalone release and otherwise downloaded once from its pin on first OCI use), and the web UI grown to parity (§16.5): ANSI-color logs, WebSocket console and SSH shell terminal, machine editing, create-form image picker and structured inputs, metrics sparklines, snapshot, clone, image and prune surfaces. Each feature landed with its own normative sections and decision-log entries. Two OCI behaviors are shipped as documented limits rather than as the contract §11.8 states: `stop` falls through to the force path because `firestone-init` answers no ACPI power button (verify 25), and `start --wait` still waits for sshd on an OCI machine, so `--no-wait` is the working spelling. Acceptance: required local gate plus `scripts/m6-kvm-e2e.py` and `scripts/m6-oci-kvm-e2e.py` green on a real x86_64 KVM host, existing e2e regression green, docs validated, release `v0.2.0` published.
...
### 25.5 Host sample (normative)

`Action::HostMetrics` answers the question a machine sample cannot: does this host have room to run another machine. It needs no machine, takes no lock, and talks to no VMM.
...
```json
{
  ...
  "cpu": {
    ...
    "load_average_5m": 0.55,
    "load_average_15m": 0.61
  },
  "memory": { "total_bytes": 16707784704, "available_bytes": 9439277056 },
  "disk": {
    "path": "/home/reader/.local/share/firestone",
    "total_bytes": 494384795648,
    "available_bytes": 201326592000
  }
}
```
...
| Field | Type | Meaning |
|---|---|---|
| `sampled_at` | RFC 3339 instant | when the host sources were read |
| `cpu.cores` | integer or null | host processor count |
| `cpu.load_average_1m`, `_5m`, `_15m` | number or null | kernel load averages from `/proc/loadavg` |
| `memory.total_bytes`, `available_bytes` | integer or null | `MemTotal` and `MemAvailable` from `/proc/meminfo`, converted from kibibytes |
| `disk.path` | string | the measured directory: Firestone's data directory |
| `disk.total_bytes`, `available_bytes` | integer or null | `statvfs` on that directory, fragment size times total and user-available blocks |

The rules of §25.2 hold unchanged: every field is present, and an unavailable figure is `null` rather than a zero. `/proc` is Linux-only, so memory and load are `null` on any other host; the core count and the disk figures are read through the standard library and `statvfs` and are available everywhere. A data directory that does not exist yet is measured through its nearest existing ancestor, which is the filesystem it will be created on.

Surfaces: `firestone system metrics` prints one three-line human table, and `GET /v1/host/metrics` returns the same payload as `application/json` with status 200. Both project `Action::HostMetrics`; neither adds a field the other lacks.
