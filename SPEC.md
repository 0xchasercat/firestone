...
- **Field errors.** Every field collects its own problem in one pass, including one message per bad mount row, and rejected values are returned verbatim so nothing typed is lost.
...
**Applies-when badges and spec drift (normative).** On a running machine the dialog badges vCPUs and memory `live`, disk `applies at next start`, and every other group `applies after restart`. A stopped machine carries no badges and one note saying every field applies the next time it starts. The detail head then reports drift in two forms, and the difference between them is stated rather than hidden. The server proves drift only for the fields `state.json` records — the image reference the running instance booted, its MAC, and the forwards it applied (§12.5) — and names them in the pill. For every other editable field there is nothing recorded to compare against, so the pill is raised by the browser after this page session saves a spec change against a running machine, and is dropped as soon as that machine is seen in any state but running, or the page is reloaded. An absent pill therefore means "nothing observable has drifted", never "the running machine matches its spec".
