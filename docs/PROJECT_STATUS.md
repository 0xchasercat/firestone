...
| M5-09 | Default nano editor fallback | `agent/default-nano-editor` | pending | in progress | M5-06 | `edit` and `create --edit` honor nonempty `VISUAL`, then `EDITOR`, then `nano` through `Cmd`; fake-`nano` process coverage and the required local gates pass |
