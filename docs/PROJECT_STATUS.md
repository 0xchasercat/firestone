...
| M5-05 | Standalone embedded helpers, AppArmor remediation, and guided create UX | `agent/standalone-helpers-create-ux` | pending | in progress | accepted Linux x86_64 MVP | Reproducible static passt/qemu-img payloads, strict include-bytes release integration, root-owned literal AppArmor remediation, effective create summaries, and default TTY wizard are implemented; final Linux static/AppArmor/KVM evidence and PR checks remain |
