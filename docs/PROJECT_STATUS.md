...
| M5-10 | Static OpenAPI 3.1 contract and Axum route drift gate | `agent/rest-openapi-spec` | — | in progress | M4-01 | `docs/openapi.json` covers the 18 authored REST operations, Unix transport, JSON/NDJSON framing, exact schemas, limits, statuses, nullability, and examples; dependency-free parsing plus behavior-level `Allow` probing detects route and method drift without a runtime endpoint or behavior change; API, docs, and full Rust gates pass |
