...
The user guide, one page per topic. Read install and the quick start in order; the rest stand on their own.
...
[`openapi.json`](openapi.json) is the OpenAPI 3.1 contract for the REST API. The design behind each behavior is in [SPEC.md](../SPEC.md); these pages are about using the tool.
