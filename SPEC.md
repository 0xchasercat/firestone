...
- `create` records the canonical image reference in `state.json`; `image.id` and source `image.sha256` are null until first start resolves immutable content. Start writes both under the machine and image-store locks before lazily invoking:
  ...
  The partial is inspected with `qemu-img info --output=json -f qcow2 PATH`. Base and overlay inspection reject dirty/corrupt qcow2 state, any external data file, malformed dependency-shaped fields, and unexpected backing metadata. An overlay must report both `backing-filename` and `full-backing-filename` as the exact owned base string, `backing-filename-format: qcow2`, and the requested virtual size before atomic publication as `<machine>/disk.qcow2`. `qemu-img info`, overlay creation, and raw conversion run through `Cmd` with respective 30 s, 60 s, and 30 min timeouts while the relevant locks remain held.
...
The API client is a small hand‑written HTTP/1.1‑over‑unix‑socket client (hyper + hyperlocal). Endpoints used in v0.1: `GET vmm.ping`, `PUT vm.create`, `PUT vm.boot`, `GET vm.info`, `PUT vm.power-button`, `PUT vm.pause`, `PUT vm.resume`, `PUT vm.shutdown`, `PUT vmm.shutdown` — all under `/api/v1/` **[verify 2]**.
...
| Image verifier provenance and generations | stable id excludes verifier and firmware; validated identical bytes atomically strengthen verifier/firmware and advance a positive per-source/architecture generation; unchecked pulls never downgrade; cache selection uses verifier-aware tri-state matching and a unique highest generation | include verifier in id; choose by timestamp; let current catalog URL or firmware invalidate every warm cache | Provenance may become stronger without changing bytes, wall clocks can repeat or move backward, and moving release URLs must not force network access during start. |
...
| qemu image inspection hardening | parse pinned 8.2.2 backing, dirty, corrupt, and format-specific data-file fields; reject hidden external dependencies; require exact backing strings and qcow2 format; bound every qemu-img command | trust create exit status; filesystem-canonicalize a reported backing path; accept unknown dependency-shaped fields | A successful qemu-img command does not prove the image is standalone or points at the owned base, and a lock-held process must have a finite failure bound. |
| Persisted image source classification | user input remains path-first; persisted state is strict HTTPS, then absolute local, then catalog without a relative existence probe; a complete local pin remains loadable after source deletion | rerun user-input heuristics for state; require the original local source forever | State must not change meaning when a relative shadow file appears, and the purpose of the owned immutable base is to survive source loss. |
