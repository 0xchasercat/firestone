...
The shim does not proxy the console or the API. `firestone console` connects to the VMM's `console.sock` directly; `firestone shell` connects to `vsock.sock` directly; `vm.info` and the liveness ping hit `api.sock` directly. Pause and resume are not v0.1 Firestone actions (§5.2, §21).
...
| [verify 6] Cloud Hypervisor v53.0 lifecycle evidence | Against tag `v53.0` commit `9ed824d6d08df3e96f7d5f50795d9449ac99f431`: bare paths are accepted for `--api-socket` ([`main.rs` 193-197, 531-552](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/9ed824d6d08df3e96f7d5f50795d9449ac99f431/cloud-hypervisor/src/main.rs#L193-L197)); `vm.power-button` injects the guest power notification but only acknowledges injection ([`api/mod.rs` 1409-1427](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/9ed824d6d08df3e96f7d5f50795d9449ac99f431/vmm/src/api/mod.rs#L1409-L1427)); with default `no_shutdown = false`, a guest exit invokes `vmm_shutdown` and breaks the VMM loop ([`vmm/src/lib.rs` 2095-2104](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/9ed824d6d08df3e96f7d5f50795d9449ac99f431/vmm/src/lib.rs#L2095-L2104)); `PUT vmm.shutdown` tears down, returns true to exit, and its HTTP handler returns 200 ([`api/mod.rs` 1870-1896](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/9ed824d6d08df3e96f7d5f50795d9449ac99f431/vmm/src/api/mod.rs#L1870-L1896), [`http_endpoint.rs` 671-695](https://github.com/cloud-hypervisor/cloud-hypervisor/blob/9ed824d6d08df3e96f7d5f50795d9449ac99f431/vmm/src/api/http/http_endpoint.rs#L671-L695)). Firestone therefore waits for child exit first, uses `vm.info` only as an additional observation, calls `vmm.shutdown` when the process remains, then verified TERM/KILL and reap. | treat power-button 204 as shutdown completion; trust `vm.info = Shutdown`; accept OpenAPI's incorrect vmm.shutdown 204 | The exact source resolves endpoint/process ordering and the v53 200 response. Whether each systemd catalog guest actually powers off after ACPI, and its timing, still requires the M1-06 KVM runtime check; this row does not close that guest-behavior portion of verify 6. |
