...
# v1.14.0 has no versioned binary release assets. Keep its immutable version and
# source archive explicit while binary distribution remains blocked.
readonly VIRTIOFSD_VERSION="v1.14.0"
readonly VIRTIOFSD_COMMIT="c2540f8db14caba81c1e37fba23fc7bf2cd7f0dd"
readonly VIRTIOFSD_RELEASE_URL="https://gitlab.com/virtio-fs/virtiofsd/-/releases/v1.14.0"
...
verify_manifest_metadata() {
    ...
    require_manifest_value dependency.virtiofsd availability source-only
...
# Upstream v1.14.0 publishes source archives but no versioned binary assets.
# The mutable main-branch CI artifact is x86_64-only and is not safe to pin.
[dependency.virtiofsd]
version = "$VIRTIOFSD_VERSION"
commit = "$VIRTIOFSD_COMMIT"
release_url = "$VIRTIOFSD_RELEASE_URL"
availability = "source-only"
missing_arches = ["x86_64", "aarch64"]
reason = "upstream release has no versioned static binary assets"
