...
**Not implemented in v0.2.** The SSH refusal and the readiness rule above are the contract; neither is wired yet. `shell`, `ssh-config` and `cp` reach their ordinary SSH paths on an OCI machine and fail by not connecting, and `start` gates SSH readiness on `cloud_init.provisioning`, which is `true` for an OCI machine and cannot be set to `false` (§10.5), so `--no-wait` is the working spelling until this lands. `docs/user-guide.md` documents that as the current behavior. Closing it means gating the readiness wait and the SSH surfaces on the image `kind` the sidecar already records.
...
| 26 | open | §11.8's SSH refusal and readiness rule hold | Neither is implemented: `wait_for_ssh_ready` is gated on `cloud_init.provisioning` rather than on the image `kind`, and `shell`/`ssh-config`/`cp` take their ordinary SSH paths on an OCI machine. `start --no-wait` is the working spelling today. Close it by branching both on the sidecar `kind` `prepare_start` already reads, with a KVM run proving that `start` returns as soon as the shim reports running and that `shell` exits 2 without a connection attempt |
