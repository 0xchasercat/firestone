...
- **The server closes with a descriptive close frame.** End of the byte stream closes with code `1000` and reason `machine stopped`. A transport failure closes with code `1011` and the failure detail, truncated on a character boundary to fit one control frame.
