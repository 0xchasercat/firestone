...
| `mkfs.ext4` | ext4 root images built directly from an OCI layer tar | Firestone static helper release from kernel.org e2fsprogs 1.47.3 linked against Alpine libarchive 3.8.3 | x86_64 musl static binary produced by the `build/helpers` recipe; publication and the `deps.toml` pin land with the helper release that carries it |
