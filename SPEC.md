...
**Live utilization (normative).** The detail page of a running machine carries a utilization strip above the tabs, and the overview machines panel carries a compact CPU figure on a running row. Both read `GET /v1/machines/{name}/metrics` and nothing else; the UI adds no metrics route and stores no history on the host.
...
**Terminal page (normative).** `GET /machines/{name}/terminal` is a full-window browser terminal for one machine. It is its own document rather than a body inside the sidebar shell — a terminal wants the whole window — with one slim bar carrying the machine name, its status, Console and Shell tab buttons, a connection pill, and a link back to the machine. It is a read like every other screen: rendering it dispatches `Action::Show`, and the byte stream it then opens is the §16.3 WebSocket transports, unchanged. The page renders for a machine in any state, because a terminal that cannot attach must say why rather than 404.
...
The machine detail head offers a **Terminal** link only while a machine is running, because both transports require a live machine.

**Content-Security-Policy exception (normative).** The terminal page response, and no other response in the application, carries `default-src 'none'; script-src 'self' 'wasm-unsafe-eval'; style-src 'self'; img-src 'self' data:; font-src 'self'; connect-src 'self'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'`. It differs from the policy above by exactly one token. `'wasm-unsafe-eval'` is what permits `WebAssembly.compile`; it is *not* `'unsafe-eval'`, so `eval` and `new Function` stay forbidden on that page too, and script sources stay same-origin-only. The consequence is that a page carrying this policy can only instantiate WebAssembly bytes it already fetched from Firestone's own asset table, so exploiting the exception still requires prior injection into the executable — the relaxation widens what a compromised page could do, never how a page becomes compromised.
...
**Read fragments.** Beside the screens, `/ui/…` serves fragments that are reads of the same shared results. `GET /ui/machines/new/images` renders the create dialog's image picker on its own so it can be re-read after a pull without rebuilding the form. Fragment routes are reads only; they are not part of the REST contract and are not advertised in `docs/openapi.json`.

**Spec fields (normative).** Every control the create dialog offers composes into the exact field name a handler already parses, in the grammar the CLI and REST already accept. The friendly controls are unnamed and carry no meaning of their own; the named field is the whole contract. `templates/ui/_spec_fields.html` holds the markup so any later spec-editing surface renders the same controls against the same names.
...
- **Repeatable rows.** Port forwards and shared folders are edited as rows and composed into the comma-joined `[proto:][bind:]HOST:GUEST` list and the newline-joined `HOST:GUEST[:ro]` list respectively, with `mounts` parsed into `MountSpec` values. Each group keeps a raw-text toggle showing the exact string that will be submitted. A raw value the rows cannot round-trip is never rewritten: the group stays raw, and the server answers.
...
**Edit surface (normative).** `GET /ui/machines/{name}/edit` renders the machine edit dialog, offered from the detail head for a machine in any state. It is a read like every other `/ui` fragment: the same `templates/ui/_spec_fields.html` controls the create dialog renders, prefilled from the machine's own spec through the same projection, plus one JSON copy of that projection carried in `data-fs-original`. The dialog writes from the browser to `/v1` and nowhere else, so the single-mutation-surface rule above is unchanged and no `/ui` route accepts a spec write.
...
- The dialog parses no Firestone grammar. Its only client-side refusals are the two the browser needs in order to build a request body at all: a field with no meaningful empty value, and a mount row that cannot be turned into a `MountSpec` object. Everything else is decided by the same validation the CLI and REST run.
...
The create dialog's snapshot name is optional and the dialog says what an empty one means: `snap-<yyyymmdd>-<hhmmss>` from the UTC instant of the request. The tab is not polled — a five-second swap under a table of destructive buttons is its own hazard — so it is re-read explicitly after each command that changes it.

**Clone (normative).** The detail head and each machines-row overflow menu offer a clone dialog: a destination name and an "empty disk" checkbox mapping to `fresh_disk: true`, writing to `POST /v1/machines/{name}/clone` and rendering its NDJSON in the same drawer every other machine action uses. On the terminal `Result` the browser navigates to `/machines/<dest>` and raises a toast, because the clone's whole point is the new machine. §24.2 refuses a source that is not `created` or `stopped` before it takes any lock, so the control is rendered disabled with that reason for every other status, and the command palette omits such a machine entirely rather than offering an entry that can only fail. The clone control never carries `data-fs-machine`: that attribute means "this button dispatches a lifecycle action", and a transitioning machine must not carry one.

**Cached images and prune (normative).** `/catalog` carries a second section below the catalog cards: the image store as it is on disk, read from `Action::ImageList`, largest first, with the reference, a shortened id whose full value stays in the row's title, the size, the pull instant, and an `OCI` badge when the sidecar's `kind` is `oci` (§8.5). An absent `kind` is a disk image, never an unknown one. Each row offers a delete against `DELETE /v1/images/{id}` behind a confirm that names the refusal it may hit — a machine or a snapshot that pins the image keeps it, and Firestone names what is holding it — and the refusal message is rendered in full beside the row, because `conflict` alone leaves the operator guessing which machine to stop.
...
| `firestone-init` | guest PID 1 for OCI machines (§10.5) | built from this repository | static musl payload embedded in Firestone, checksum-verified and injected into the packed rootfs at `/sbin/firestone-init`; a build with no payload reports kind `dependency` |
...
`firestone-init` is the one embedded payload that is a Firestone build artifact rather than a third-party download, and it is never materialized into `<data>/bin`: its only consumer is the rootfs injection of §8.5, which writes the bytes straight into the merged tar. `build.rs` verifies it against `deps.toml` exactly like the other payloads, but treats it as optional in both directions: a `[dependency.firestone-init]` entry without the built asset, and an asset without an entry, are both build failures, while neither present is the ordinary development build. Until the standalone `firestone-init` release exists and is pinned, every such build answers the injection feed with one `dependency` error naming the missing release instead of silently injecting nothing. Building that release reproducibly inside the existing `build/firestone` container, publishing it as its own release asset, and pinning it in `deps.toml` is an orchestrator follow-up, like `mkfs.ext4`; the runbook is `docs/verification/firestone-init-release.md`.
...
| Checking `firestone-init` before the manifest (M6-15) | The payload feed is called first, before the registry is contacted at all, and its failure is re-reported with a hint naming the standalone release and the `deps.toml` pin | check it when the merge needs it; inject a placeholder and fail at boot; probe the payload once at process start | The payload is a property of the build, not of the network, so a build that cannot produce a bootable image should say so in milliseconds rather than after several gigabytes of layer downloads that will be discarded. Failing at the merge would make the wasted work the default experience on every development build, which is exactly the configuration that lacks the payload today. |
